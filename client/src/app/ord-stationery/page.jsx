...
export default function page() {
    ...
    let { token, stationeryCart, stationeryCartItems } = useContext(loginContext)
    ...
    let totle = 0
    stationeryCart.forEach((price) => totle += price.price)
    stationeryCart.forEach((items) => console.log(items.category))
    ...
    let removeCart = (id) => {
        axios.delete(`${APIURL}/ship/stationery-cart/delete-cart/${id}`)
            .then((res) => res.data)
            .then((finalData) => {
    ...
    useEffect(() => {
        const items = stationeryCart.map(item => ({
            id: item._id,
            title: item.title,
            image: item.image,
            price: item.price,
    ...
    let order = () => {
        ...
        if (token) {
            ...
            axios.post(`${APIURL}/ship/order/add-stationery-order`, { orderItems, orderType: "stationery" }, {
                headers: {
            ...
            <div className='p-8'>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-2xl font-bold text-slate-900" id="totalItemsCount">{stationery.length}</div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-2xl font-bold text-slate-900" id="cartItemsCount">{stationeryCart.length}</div>
                    ...
                    stationery.length > 0 ?
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" id="cateringItemsGrid">
                            {stationery.map((items, index) => (
                                ...
                                //     </div>
                                // </div>
                                <StationeryProduct items={items} index={index} />
                            ))}
                        </div>
                        :
                                    ...
                                    <div className="mt-8">
                                        <div className="flow-root">
                                            <ul role="list" className="-my-6 divide-y divide-gray-200">
                                                ...
                                                {stationeryCart.length > 0 ?
                                                    stationeryCart.map((items, index) => (
